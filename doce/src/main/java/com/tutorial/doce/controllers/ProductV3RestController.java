...
import com.tutorial.doce.dtos.ProductCollection;
...
@CrossOrigin(origins = "^")
public class ProductV3RestController {
    ...
    public ResponseEntity<ProductCollection> index() {
        ...
        ProductCollection collection = new ProductCollection(dtos, new ProductCollection.AdditionalData("Mega Store", "http://localhost:8080/api/v3/products"));
    ...
    public ResponseEntity<ProductCollection> paginate(@RequestParam(defaultValue = "1") int page) {
        ...
        if (start >= products.size()){
            ...
            dtos = products.subList(start, end).stream().map(p -> new ProductDTO(p.getId(), p.getName(), p.getPrice())).collect(Collectors.toList());
        }
        ProductCollection collection = new ProductCollection(dtos, new ProductCollection.AdditionalData("Mega Store", "http://localhost:8080/api/v3/products"));
        return ResponseEntity.ok(collection);
    }
